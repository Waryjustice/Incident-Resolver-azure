...
class DiagnosisAgent:
    def __init__(self):
        ...
        if gemini_api_key:
            try:
                ...
                print("[Diagnosis Agent] ** Gemini AI client initialized (" + self.model_name + ")")
            except Exception as e:
                print("[Diagnosis Agent] ** Could not initialize Gemini client: " + str(e))
        else:
            print("[Diagnosis Agent] ** GEMINI_API_KEY not set — AI diagnosis unavailable (using rule-based fallback)")
    ...
    async def diagnose_incident(self, incident):
        ...
        try:
            ...
            print(f"[Diagnosis Agent] [ERROR] Diagnosis failed: " + str(e))
            return None
    ...
    async def determine_root_cause(self, incident, context, similar_incidents, log_analysis):
        ...
        if self._gemini_model:
            ...
            try:
                ...
                print("[Diagnosis Agent] 🤖 AI diagnosis: " + root_cause.get("description", ""))
                return root_cause
            except json.JSONDecodeError as e:
                print("[Diagnosis Agent] ⚠️  Could not parse AI response as JSON: " + str(e))
            except Exception as e:
                print("[Diagnosis Agent] ⚠️  AI call failed (" + str(e) + "), using rule-based fallback")
...
Based on this data, identify the root cause."""
    ...
    async def start_listening(self):
        ...
        if not self.servicebus_connection_string:
            print("[Diagnosis Agent] ** AZURE_SERVICEBUS_CONNECTION_STRING not set")
            return

        self.is_listening = True
        print("[Diagnosis Agent] [INFO] Starting to listen for messages on queue: " + self.input_queue_name)
        ...
        while self.is_listening:
            try:
                            ...
                            try:
                                ...
                                for message in messages:
                                    try:
                                        ...
                                        print("[Diagnosis Agent] [INFO] Received incident: " + incident_data['id'])
                                        ...
                                        if diagnosis:
                                            ...
                                            print("[Diagnosis Agent] [INFO] Diagnosis forwarded to resolution queue for: " + incident_data['id'])
                                        else:
                                            print("[Diagnosis Agent] [WARNING] Diagnosis returned None for: " + incident_data['id'])

                                    except json.JSONDecodeError as e:
                                        print("[Diagnosis Agent] [ERROR] Failed to parse message body: " + str(e))
                                        await receiver.dead_letter_message(message, reason="InvalidJSON")
                                    except Exception as e:
                                        print("[Diagnosis Agent] [ERROR] Failed to process message: " + str(e))
                ...
                print("[Diagnosis Agent] [ERROR] Service Bus connection lost: " + str(e))
        ...
        try:
            async with AsyncServiceBusClient.from_connection_string(
                ...
                async with client.get_queue_sender(self.output_queue_name) as sender:
                    ...
                    print("[Diagnosis Agent] [SUCCESS] Diagnosis " + diagnosis['incident_id'] + " sent to resolution agent")

        except Exception as e:
            print("[Diagnosis Agent] ** Failed to send diagnosis to resolution agent: " + str(e))
...
if __name__ == "__main__":
    ...
    if "--test" in sys.argv or os.getenv("TEST_MODE", "false").lower() == "true":
        ...
        print("[Diagnosis Agent] Injecting test incident: " + _json.dumps(test_incident, indent=2) + "\n")
        try:
